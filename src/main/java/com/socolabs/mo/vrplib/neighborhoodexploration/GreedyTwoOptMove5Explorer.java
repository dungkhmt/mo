...
import com.socolabs.mo.vrplib.moves.TwoOptMove5;
...
public class GreedyTwoOptMove5Explorer implements INeighborhoodExploration {
    ...
    public GreedyTwoOptMove5Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                ...
                if (cr != ar) {
                    for (VRPPoint y = cr.getStartPoint(); y != cr.getEndPoint(); y = y.getNext()) {
                        for (VRPPoint x = ar.getStartPoint(); x != ar.getEndPoint(); x = x.getNext()) {
                            if (vr.exploreTwoOptMove5(x, y)) {
                                LexMultiValues eval = objectiveF.evaluate();
                                if (objectiveF.compare(eval, 0) < 0) {
                                    TwoOptMove5 move = new TwoOptMove5(vr, x, y, eval);
