...
import com.socolabs.mo.vrplib.moves.ThreeOptMove6;
...
public class GreedyThreeOptMove6Explorer implements INeighborhoodExploration {
    ...
    public GreedyThreeOptMove6Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
            ...
            for (VRPPoint x = cr.getStartPoint().getNext(); x != cr.getEndPoint(); x = x.getNext()) {
                for (VRPPoint y = x.getNext(); y != cr.getEndPoint(); y = y.getNext()) {
                    for (VRPPoint z = y.getNext(); z != cr.getEndPoint(); z = z.getNext()) {
                        if (vr.exploreThreeOptMove6(x, y, z)) {
                            LexMultiValues eval = objectiveF.evaluate();
                            if (objectiveF.compare(eval, 0) < 0) {
                                ThreeOptMove6 move = new ThreeOptMove6(vr, x, y, z, eval);
