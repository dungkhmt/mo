...
import com.socolabs.mo.vrplib.moves.ThreeOptMove2;
...
public class GreedyThreeOptMove2Explorer implements INeighborhoodExploration {
    ...
    public GreedyThreeOptMove2Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
            ...
            for (VRPPoint x = cr.getStartPoint().getNext(); x != cr.getEndPoint(); x = x.getNext()) {
                for (VRPPoint y = x.getNext(); y != cr.getEndPoint(); y = y.getNext()) {
                    for (VRPPoint z = y.getNext(); z != cr.getEndPoint(); z = z.getNext()) {
                        if (vr.exploreThreeOptMove2(x, y, z)) {
                            LexMultiValues eval = objectiveF.evaluate();
                            if (objectiveF.compare(eval, 0) < 0) {
                                ThreeOptMove2 move = new ThreeOptMove2(vr, x, y, z, eval);
