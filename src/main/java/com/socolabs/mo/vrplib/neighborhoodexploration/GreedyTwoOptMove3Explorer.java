...
import com.socolabs.mo.vrplib.moves.TwoOptMove3;
...
public class GreedyTwoOptMove3Explorer implements INeighborhoodExploration {
    ...
    public GreedyTwoOptMove3Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                ...
                if (cr != ar) {
                    for (VRPPoint y = cr.getStartPoint(); y != cr.getEndPoint(); y = y.getNext()) {
                        for (VRPPoint x = ar.getStartPoint(); x != ar.getEndPoint(); x = x.getNext()) {
                            if (vr.exploreTwoOptMove3(x, y)) {
                                LexMultiValues eval = objectiveF.evaluate();
                                if (objectiveF.compare(eval, 0) < 0) {
                                    TwoOptMove3 move = new TwoOptMove3(vr, x, y, eval);
