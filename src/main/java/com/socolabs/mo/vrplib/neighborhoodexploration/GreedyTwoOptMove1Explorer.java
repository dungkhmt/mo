...
import com.socolabs.mo.vrplib.moves.TwoOptMove1;
...
public class GreedyTwoOptMove1Explorer  implements INeighborhoodExploration {
    ...
    public GreedyTwoOptMove1Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                ...
                if (cr != ar) {
                    for (VRPPoint y = cr.getStartPoint(); y != cr.getEndPoint(); y = y.getNext()) {
                        for (VRPPoint x = ar.getStartPoint(); x != ar.getEndPoint(); x = x.getNext()) {
                            if (vr.exploreTwoOptMove1(x, y)) {
                                LexMultiValues eval = objectiveF.evaluate();
                                if (objectiveF.compare(eval, 0) < 0) {
                                    TwoOptMove1 move = new TwoOptMove1(vr, x, y, eval);
