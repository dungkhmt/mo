...
import com.socolabs.mo.vrplib.moves.ThreeOptMove8;
...
public class GreedyThreeOptMove8Explorer implements INeighborhoodExploration {
    ...
    public GreedyThreeOptMove8Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
            ...
            for (VRPPoint x = cr.getStartPoint().getNext(); x != cr.getEndPoint(); x = x.getNext()) {
                for (VRPPoint y = x.getNext(); y != cr.getEndPoint(); y = y.getNext()) {
                    for (VRPPoint z = y.getNext(); z != cr.getEndPoint(); z = z.getNext()) {
                        if (vr.exploreThreeOptMove8(x, y, z)) {
                            LexMultiValues eval = objectiveF.evaluate();
                            if (objectiveF.compare(eval, 0) < 0) {
                                ThreeOptMove8 move = new ThreeOptMove8(vr, x, y, z, eval);
