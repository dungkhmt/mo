...
import com.socolabs.mo.vrplib.moves.TwoOptMove2;
...
public class GreedyTwoOptMove2Explorer implements INeighborhoodExploration {
    ...
    public GreedyTwoOptMove2Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                ...
                if (cr != ar) {
                    for (VRPPoint y = cr.getStartPoint(); y != cr.getEndPoint(); y = y.getNext()) {
                        for (VRPPoint x = ar.getStartPoint(); x != ar.getEndPoint(); x = x.getNext()) {
                            if (vr.exploreTwoOptMove2(x, y)) {
                                LexMultiValues eval = objectiveF.evaluate();
                                if (objectiveF.compare(eval, 0) < 0) {
                                    TwoOptMove2 move = new TwoOptMove2(vr, x, y, eval);
