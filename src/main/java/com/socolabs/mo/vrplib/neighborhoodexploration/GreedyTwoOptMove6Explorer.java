...
import com.socolabs.mo.vrplib.moves.TwoOptMove6;
...
public class GreedyTwoOptMove6Explorer implements INeighborhoodExploration {
    ...
    public GreedyTwoOptMove6Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                ...
                if (cr != ar) {
                    for (VRPPoint y = cr.getStartPoint(); y != cr.getEndPoint(); y = y.getNext()) {
                        for (VRPPoint x = ar.getStartPoint(); x != ar.getEndPoint(); x = x.getNext()) {
                            if (vr.exploreTwoOptMove6(x, y)) {
                                LexMultiValues eval = objectiveF.evaluate();
                                if (objectiveF.compare(eval, 0) < 0) {
                                    TwoOptMove6 move = new TwoOptMove6(vr, x, y, eval);
