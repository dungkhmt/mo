...
public class GreedyCrossExchangeMove1Explorer implements INeighborhoodExploration {
    ...
    public GreedyCrossExchangeMove1Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                                    ...
                                    for (VRPPoint y2 = x1.getNext(); y2 != ar.getEndPoint(); y2 = y2.getNext()) {
                                        if (vr.exploreCrossExchangeMove1(x1, y1, x2, y2)) {
                                            ...
                                            if (objectiveF.compare(eval, 0) < 0) {
                                                CrossExchangeMove1 move = new CrossExchangeMove1(vr, x1, y1, x2, y2, eval);
