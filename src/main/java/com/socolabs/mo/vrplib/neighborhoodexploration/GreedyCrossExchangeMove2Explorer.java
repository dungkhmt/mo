...
import com.socolabs.mo.vrplib.moves.CrossExchangeMove2;
...
public class GreedyCrossExchangeMove2Explorer implements INeighborhoodExploration {
    ...
    public GreedyCrossExchangeMove2Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                            ...
                            for (VRPPoint y1 = x1.getNext(); y1 != cr.getEndPoint(); y1 = y1.getNext()) {
                                for (VRPPoint x2 = ar.getStartPoint().getNext(); x2.getNext() != ar.getEndPoint(); x2 = x2.getNext()) {
                                    for (VRPPoint y2 = x2.getNext(); y2 != ar.getEndPoint(); y2 = y2.getNext()) {
                                        if (vr.exploreCrossExchangeMove2(x1, y1, x2, y2)) {
                                            LexMultiValues eval = objectiveF.evaluate();
                                            if (objectiveF.compare(eval, 0) < 0) {
                                                CrossExchangeMove2 move = new CrossExchangeMove2(vr, x1, y1, x2, y2, eval);
    ...
    public String name() {
        return "GreedyCrossExchangeMove2Explorer";
    }
}
