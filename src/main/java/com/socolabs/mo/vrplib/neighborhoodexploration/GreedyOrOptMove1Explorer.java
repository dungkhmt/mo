...
import com.socolabs.mo.vrplib.moves.OrOptMove1;
...
public class GreedyOrOptMove1Explorer implements INeighborhoodExploration {
    ...
    public GreedyOrOptMove1Explorer(VRPVarRoutes vr, ExplorationSelector selector) {
        this.vr = vr;
                    ...
                    for (VRPPoint x1 = cr.getStartPoint().getNext(); x1.getNext() != cr.getEndPoint(); x1 = x1.getNext()) {
                        for (VRPPoint x2 = x1.getNext(); x2 != cr.getEndPoint(); x2 = x2.getNext()) {
                            for (VRPPoint y = ar.getStartPoint(); y != ar.getEndPoint(); y = y.getNext()) {
                                if (vr.exploreOrOptMove1(x1, x2, y)) {
                                    LexMultiValues eval = objectiveF.evaluate();
                                    if (objectiveF.compare(eval, 0) < 0) {
                                        OrOptMove1 move = new OrOptMove1(vr, x1, x2, y, eval);
