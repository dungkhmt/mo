...
public class MaxAccumulatedWeightPoints implements IVRPFunction {
    ...
    public MaxAccumulatedWeightPoints(AccumulatedWeightPoints acc) {
        vr = acc.getVarRoutes();
        this.acc = acc;
        init();
    }
...
//            value += acc.getWeightValueOfPoint(route.getEndPoint());
    ...
    public boolean verify() {
        ...
        for (VRPRoute r : vr.getAllRoutes()) {
            v = Math.max(v, acc.getWeightValueOfPoint(r.getEndPoint()));
        }
        if (value != tmpValue) {
