...
public class Agent extends Thread {
	...
	public static final String urlUpdateServicePoints = "http://localhost:8080/updateServicePoints";
	...
	public void createObject(){
		String json = "{\"id\":\"" + this.name + "\",\"lat\":" + 0 + ",\"lng\":" + 0 + "}"; 
		try{
			String rs = execPost(urlCreateTruck, json);
			System.out.println(name + "::createObject , json = " + json + ", rs = " + rs);
	...
	public void updatePlannedRoute(){
		//String json = "{\"id\":\"" + this.name + "\",\"lat\":" + 0 + ",\"lng\":" + 0 + "}"; 
		UpdatePlannedRouteTruckInput input = new UpdatePlannedRouteTruckInput(this.name, route);
		String json = ApiController.gson.toJson(input);
		try{
			String rs = execPost(urlUpdatePlannededRouteTruck, json);
			System.out.println(name + "::updatePlannedRoute , json = " + json + ", rs = " + rs);
	...
	public boolean samePosition(double lat1, double lng1, double lat2, double lng2){
		double d = ApiController.GMQ.computeDistanceHaversine(lat1, lng1, lat2, lng2);
		return d < EPS;
	}
		...
		try{
			for(int i = 0; i < route.getPaths().length; i++){
				...
				for(int j = 0; j < p.getPoints().length; j++){
					...
					String json1 = "{\"id\":\"" + this.name + "\",\"lat\":" + pj.getLat() + 
							",\"lng\":" + pj.getLng() + "}"; 
					
					String rs1 = execPost(urlUpdateLocation, json1);
					double d = ApiController.GMQ.computeDistanceHaversine(pj.getLat(), pj.getLng(), next_p.getLat(), next_p.getLng());
					System.out.println(name + "::updateLocation step (" + i + "," + j + "), distance to next service point = " + d + ", json1 = " + json1 + ", rs1 = " + rs1);
					/*
					if(samePosition(pj.getLat(), pj.getLng(), next_p.getLat(),next_p.getLng())){
						String objectId = this.name;
						nextServicePointIndex++;
						int cnt = route.getPoints().length - nextServicePointIndex;
						RouteVRPInputPoint[] points = new RouteVRPInputPoint[cnt];
						for(int k = 0; k < cnt; k++) points[k] = route.getPoints()[k+nextServicePointIndex]; 
						UpdateServicePointsInput input = new UpdateServicePointsInput(objectId, points);
						String json2 = ApiController.gson.toJson(input);
						String rs2 = execPost(urlUpdateServicePoints, json2);
						System.out.println(name + "::updateLocation, CALL updateServicePoints, nextServicePointIndex = " + nextServicePointIndex + ", json2 = " + json2 + ", rs2 = " + rs2);
					}
					*/
					
					Thread.sleep(1000);
				}
				PerformServiceTruckInput inp = new PerformServiceTruckInput(this.name);
	...
	public void runRouteFromPointToPoint(){
		...
		try{
		String json = "{\"fromID\":0,\"toID\":0,\"fromPoint\":\"" + moveFromPoint + "\",\"toPoint\":\"" + moveToPoint + "\"}";
		
			String rs = execPost(urlShortestPath, json);
			Gson gson = new Gson();
			Path p = gson.fromJson(rs, Path.class);
			for(int i = 0; i < p.getPoints().length; i++){
				String json1 = "{\"id\":\"" + this.name + "\",\"lat\":" + p.getPoints()[i].getLat() + 
						",\"lng\":" + p.getPoints()[i].getLng() + "}"; 
				
				String rs1 = execPost(urlUpdateLocation, json1);
				System.out.println(name + "::updateLocation step " + i + ", json1 = " + json1 + ", rs1 = " + rs1);
				Thread.sleep(1000);
		...
		for(int i = 0; i < 1000; i++){
			...
			String json = "{\"id\":\"" + this.name + "\",\"lat\":" + a + ",\"lng\":" + b + "}"; 
			try{
				String rs = execPost(urlUpdateLocation, json);
				System.out.println(name + "::updateLocation step " + i + ", json = " + json + ", rs = " + rs);
				Thread.sleep(1000);
